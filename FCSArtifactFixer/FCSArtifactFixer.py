...
class Parallel_scheduler():
    # Relatively simple class wrapping FCSArtifactFixer to run the 
    ...
    def run_standard_pipeline_all_channels(in_path,
                                           ...
                                           job_name = ''):
        '''
        Load a file into FCSArtifactFixer, and run the standard pipeline in all
        ...
        job_name : 
            ...
            FCSArtifactFixer.write_to_log() as the "calling_function" name from 
        ...
        fixer = FCSArtifactFixer(photon_data = photon_data, 
                                   out_path = out_path,
                                   tau_min = tau_min,
                                   tau_max = tau_max,
                                   sampling = sampling,
...
class FCSArtifactFixer():
    ...
    def correlation_apply_filters(self, 
        ...
        ''' 
        Perform a very basic correlation function calculation, without filters.
            Mostly for quick checks, or for preliminary automated 
            calculations that do not require "the nicest 
            correlation functions possible". Stripped down to be fast without much overhead.
        INPUTS:
    ...
    def get_Wohland_SD(self, 
        ...
        Based on Wohland, Vogel, Rigler Biophysical J 2001 (method 4 proposed in that paper)
    ...
    def get_correlation_uncertainty(self,
        ...
        if not default_uncertainty_method in ['Wohland', 'Bootstrap']:
        ...
        if default_uncertainty_method == 'Wohland':
            ...
            # implies default_uncertainty_method == 'Bootstrap':
            sd_cc = self.get_bootstrap_SD(channels_spec_1,
                                        channels_spec_2, 
...
#%% Instance methods - BLEACHING/DRIFT CORRECTION
    ...
    def run_mse_filter(self,
        ...
        minimum_window_length:
            ...
            in which to separate the trace. In seconds.
        mse_fold_threshold : 
            OPTIONAL float with default 10. Discarding of segmentas is based on
    ...
    def get_tcspc_histogram(self,
        ...
        two ways to do this:
            1. Supplying a channels_spec structure (actually always required, 
               ...
               the TTTR data loaded into the FCSArtifactFixer instance.
    ...
    def run_standard_pipeline(self,
        ...
        if use_drift_correction:
            ...
            _ = self.polynomial_detrending_rss(time_trace_counts, 
                                               time_trace_t, 
                                               channels_spec_norm_ch1,
                                               use_burst_removal = use_burst_removal,
                                               calling_function = 'run_standard_pipeline',
